...
public class EnumWrapper<T extends Enum<T>> implements ConfigElementTypeWrapper<Enum<T>, Enum<T>, EnumWrapper.Properties<T>> {
	...
	@Override
	public Enum<T> resolve(ConfigContent category, String qualifier, com.mmodding.library.config.impl.element.builtin.EnumWrapper.Properties<T> properties) {
		return Enum.valueOf(properties.enumClass(), category.string(qualifier).toUpperCase());
	}

	@Override
	public void modify(MutableConfigContent mutable, String qualifier, com.mmodding.library.config.impl.element.builtin.EnumWrapper.Properties<T> properties, Enum<T> element) {
