package com.mmodding.library.resource.impl.client.cosmetic.renderer;

import com.mmodding.library.resource.api.client.cosmetic.Cosmetic;
import com.mmodding.library.resource.api.client.cosmetic.renderer.CosmeticRenderer;
...
public class SuitCosmeticRenderer extends CosmeticRenderer {
	...
	public void render(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, ItemStack stack, HumanoidRenderState state, EquipmentSlot slot, int light, HumanoidModel<HumanoidRenderState> contextModel) {
		...
		if (bodyModel != null) {
			...
			submitNodeCollector.submitModel(bodyModel, state, poseStack, renderType, state.lightCoords, overlayCoords, state.outlineColor);
			poseStack.popPose();
		}
		if (leftSleeveModel != null && rightSleeveModel != null) {
			...
			submitNodeCollector.submitModel(leftSleeveModel, state, poseStack, renderType, state.lightCoords, overlayCoords, state.outlineColor);
			poseStack.popPose();
			poseStack.pushPose();
			contextModel.translateToHand(state, HumanoidArm.RIGHT, poseStack);
			poseStack.translate(0.3125f, -0.75f, 0.0f);
			submitNodeCollector.submitModel(rightSleeveModel, state, poseStack, renderType, state.lightCoords, overlayCoords, state.outlineColor);
			poseStack.popPose();
		}
		poseStack.popPose();
	}
