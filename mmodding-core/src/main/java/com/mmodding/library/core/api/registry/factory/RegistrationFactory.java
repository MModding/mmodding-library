...
public interface RegistrationFactory<T> {
	 ...
	 * @return the newly created registration factory
	 */
	static <T> RegistrationFactory<T> create(ResourceKey<? extends Registry<T>> registry, BootstrapContext<T> context, String namespace) {
		return new RegistrationFactoryImpl<>(registry, context, namespace);
	}
