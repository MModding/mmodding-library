...
public class AdvancedFreezeTopLayerFeature extends Feature<AdvancedFreezeTopLayerFeature.Config> {
			...
			for (int zOffset = 0; zOffset < 16; zOffset++) {
				...
				for (int yOffset = 0; yOffset < context.getConfig().depthCoverage.get(context.getRandom()); yOffset++) {
					...
					if (biome.canSetIce(world, blockPos, false)) {
						world.setBlockState(
							blockPos,
							context.getConfig().iceBlock.get(context.getRandom(), blockPos),
							Block.NOTIFY_LISTENERS
						);
					}

					if (biome.canSetSnow(world, abovePosition)) {
						world.setBlockState(abovePosition, context.getConfig().snowLayer.get(context.getRandom(), abovePosition), Block.NOTIFY_LISTENERS);
	...
	private <E extends Enum<E> & StringIdentifiable> void updateUpsideSensitiveBlock(BlockState state, StructureWorldAccess world, BlockPos pos) {
		if (state.getBlock() instanceof UpsideSensitiveBlock<?>) {
			...
			world.setBlockState(
				...
				state.with(
					...
					upsideSensitive.getInfluence(world.getBlockState(new BlockPos(pos).up()))
				),
				Block.NOTIFY_LISTENERS
			);
		}
